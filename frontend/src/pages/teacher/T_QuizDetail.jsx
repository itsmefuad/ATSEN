...
const T_QuizDetail = () => {
  ...
  const handleGradeQuiz = async (studentId) => {
    try {
      ...
      const existingGrade = grades.find(g => g.student._id === studentId && g._id);
      ...
      if (existingGrade) {
        ...
        // Create new grade
        response = await api.post(`/quiz-grades/${assessmentId}/grade/${studentId}`, {
          marks: parseFloat(gradingData.marks),
          teacherFeedback: gradingData.teacherFeedback
        });
      }
          ...
          <div className="card-body">
            ...
            {grades.length === 0 ? (
              <div className="text-center py-8">
                <p className="text-base-content/60">No students enrolled in this room</p>
              </div>
            ) : (
              <div className="space-y-4">
                {grades.map((gradeData, index) => (
                  <div key={gradeData.student._id || index} className="border border-base-300 rounded-lg p-4">
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-4 flex-1">
                        <div className="avatar">
                          <div className="w-12 h-12 rounded-full bg-primary text-primary-content flex items-center justify-center">
                            <span className="text-lg font-bold">
                              {gradeData.student?.name?.charAt(0)?.toUpperCase() || 'U'}
                            </span>
                          </div>
                        </div>
                        <div className="flex-1">
                          <div className="flex items-center gap-3 mb-1">
                            <h3 className="font-semibold">{gradeData.student?.name || 'Unknown Student'}</h3>
                            {gradeData.isGraded && (
                          ...
                          <p className="text-sm text-base-content/60">{gradeData.student?.email || 'No email'}</p>
                          {gradeData.isGraded && (
                            <div className="flex items-center gap-1 text-sm text-base-content/60 mt-1">
                              <span>Graded on {formatDate(gradeData.gradedAt)}</span>
                            </div>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3">
                        {gradingStudent === gradeData.student._id ? (
                          <div className="flex gap-2">
                            <button
                              onClick={() => handleGradeQuiz(gradeData.student._id)}
                    ...
                    {gradeData.isGraded && gradingStudent !== gradeData.student._id && (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div>
                            ...
                            <div className="text-sm text-base-content/60 mt-1">
                              {((gradeData.marks / getMaxMarks()) * 100).toFixed(1)}%
                            </div>
                          </div>
                          {gradeData.teacherFeedback && (
                            <div>
                              <p className="text-sm font-medium mb-1">Teacher Feedback:</p>
                              <p className="text-sm">{gradeData.teacherFeedback}</p>
                            </div>
                          )}
                        </div>
                      </div>
