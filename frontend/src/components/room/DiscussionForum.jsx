...
import PendingDiscussions from "./PendingDiscussions";
...
const DiscussionForum = ({ roomId }) => {
  ...
  const fetchForumContent = async () => {
    try {
      ...
      // For instructors, fetch all content including unapproved discussions
      const response = await api.get(
        `/forum-content/room/${roomId}?userRole=instructor`
      );
  ...
  const handleDiscussionApproved = (approvedDiscussion) => {
    // Add the approved discussion to the main forum content
    setForumContent((prev) => {
      const updatedContent = [...prev, approvedDiscussion];
      return updatedContent.sort((a, b) => {
        if (a.isPinned && !b.isPinned) return -1;
        if (!a.isPinned && b.isPinned) return 1;
        if (a.isPinned && b.isPinned) {
  ...
  // Separate announcements and approved discussions for teachers to see both
  const announcements = forumContent.filter(
    (item) => item.contentType === "announcement"
  );
  const approvedDiscussions = forumContent.filter(
    (item) => item.contentType === "discussion" && item.isApproved
  );
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Pending Discussions for Review */}
      <div className="space-y-4">
        <PendingDiscussions
          roomId={roomId}
          onDiscussionApproved={handleDiscussionApproved}
        />
      </div>

      {/* Approved Student Discussions Section */}
      <div className="space-y-4">
        <div className="flex items-center gap-2">
        ...
        {approvedDiscussions.length === 0 ? (
          <div className="text-center py-8">
            ...
            <h4 className="text-md font-medium text-base-content/70 mb-1">
              No approved discussions yet
            </h4>
            <p className="text-sm text-base-content/50">
              Student discussions will appear here after approval.
            </p>
          </div>
        ) : (
          <div className="space-y-4">
            {approvedDiscussions.map((discussion, index) => (
              <div
                key={discussion._id}
                className={`transition-all duration-700 ease-in-out ${
